...
void create_excluir_usuario_page(const char *mensagem_erro) {
    char bloco_erro[256] = "";
    ...
    if (file < 0) {
        snprintf(conteudo_tabela, sizeof(conteudo_tabela),
                 "<tr><td colspan=\"4\">Erro ao abrir o arquivo de usuários.</td></tr>");
    } else {
        char buffer[128];
        char linha[160] = {0};
        int linha_pos = 0;
        ...
        for (int i = 0; i < total_linhas; i++) {
            char nome[64], tipo[16], usuario[32], hash[65];
            if (sscanf(linhas[i], "%63[^,],%15[^,],%31[^,],%64[^\n]", nome, tipo, usuario, hash) == 4) {
                char linha_html[512];
                snprintf(linha_html, sizeof(linha_html),
                    "<tr>"
                    "<td>%s</td><td>%s</td><td>%s</td>"
                    "<td>"
                    "<form method=\"POST\" action=\"/excluir-usuario\" onsubmit=\"return confirm('Tem certeza que deseja excluir este usuário?');\">"
    ...
    snprintf(http_response, sizeof(http_response),
        ...
        "  </form>\n"
        "  <div class=\"marca\">Excluir Usuário</div>\n"
        "</div>\n"
        ...
        "<thead><tr><th>Nome</th><th>Tipo</th><th>Usuário</th><th>Ações</th></tr></thead>\n"
        "<tbody>%s</tbody>\n"
        "</table>\n"
        "</div>\n"
        "</div>\n"
